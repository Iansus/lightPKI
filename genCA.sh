...
# Directory init
cp initDirectory.sh openssl.cnf genCA.sh genCert.sh genChain.sh "$CA_NAME"
cd "$CA_NAME"
./initDirectory.sh
cd ..
...
openssl ca -create_serial -out "certs/$CA_NAME.pem" -days 1097 -extensions v3_ca -config openssl.cnf -infiles "req/$CA_NAME.csr"
